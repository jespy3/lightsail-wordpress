...
docker exec -i wordpress_db mysql -u root -pjibpass -e "\
  USE wordpress; \
	UPDATE wp_options SET option_value = \"http://$PUBLICIP:80\" WHERE option_name = 'siteurl'; \
	UPDATE wp_options SET option_value = \"http://$PUBLICIP:80\" WHERE option_name = 'home';"
